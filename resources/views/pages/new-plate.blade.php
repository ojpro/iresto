        ...
        <form action="{{ route('plates.store') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="mb-6">
                ...
                <input type="text" id="name"
                       ...
                       placeholder="PLate Name" name="name" value="{{ old('name') }}" required />
                @error('name')
            ...
            <div class="">
                <div class="w-full">
                    {{-- TODO: limit number of files choisen --}}
                        <input class="form-control block w-full px-3 py-1.5 text-base font-normal text-gray-700 bg-white bg-clip-padding border border-solid border-gray-300 rounded transition ease-in-out m-0 focus:text-gray-700 focus:bg-white focus:border-blue-600 focus:outline-none"  value="{{ old('images') }}" name="images[]" multiple type="file" id="formFile">
                </div>
            ...
            <div class="my-6">
                ...
                <input type="text" id="price"
                       class="block w-full rounded-md border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 dark:text-white dark:placeholder-gray-400 dark:focus:border-blue-500 dark:focus:ring-blue-500"
                       placeholder="PLate Price"  value="{{ old('price') }}" name="price" required />
                @error('price')
                <span class="font-light text-sm text-red-400">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-6">
                ...
                <textarea id="message" rows="4"
                          ...
                          placeholder="Plate Description..." name="description"> {{ old('description') }}</textarea>
                @error('description')
                <span class="font-light text-sm text-red-400">{{ $message }}</span>
                @enderror
            </div>
